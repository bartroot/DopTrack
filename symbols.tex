...
\newcommand*{\acrodefcompvec}[2]{
	...
	\acrosdefymplural	{#1.y.#2}	{\acs{#1}\acs{y}\acs{#2}}{\acl{y} of the \acl{#2} \acl {#1}}
...
%
\acrosymni			{a0}				{$A_0$}					{vertical amplitude}
\acrosymni			{b0}				{$B_0$}					{cross-track amplitude}
\acrosymni			{alpha}			{$\alpha$}			{vertical phase}
\acrosymni			{beta}			{$\beta$}				{horizontal phase}
\acrosymni			{xoff}			{$x_{\rm off}$}	{vertical offset}
...
\acrodefsymplural	{noise.gf}		{${\bm\Delta}^{\rm\left(V\right)}$}				{gravity field model error}
